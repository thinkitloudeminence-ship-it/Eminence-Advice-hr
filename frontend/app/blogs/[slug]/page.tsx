...
export default function BlogDetailPage() {
  ...
  return (
    <>
      <NextSeo
        title={`${blog.title} - Expert Career Insights & Guidance | Eminance Advice`}
        description={`Read ${blog.title} for expert career advice, tips, and insights. Learn about ${blog.category?.toLowerCase() || 'career topics'} and get practical guidance for your career growth.`}
        canonical={`https://eminenceadvice.com/blogs/${slug}`}
        openGraph={{
          url: `https://eminenceadvice.com/blogs/${slug}`,
          title: `${blog.title} - Expert Career Insights | Eminance Advice`,
          description: blog.excerpt || `Read expert career advice and insights on ${blog.category}.`,
          images: [
            {
              url: blog.featuredImage?.url || 'https://eminenceadvice.com/blog-og.jpg',
              width: 1200,
              height: 630,
              alt: blog.title,
            },
        ...
        additionalMetaTags={[
          { name: 'keywords', content: `${blog.category?.toLowerCase() || 'career'}, career advice, ${blog.tags?.join(', ') || ''}, professional tips, career guidance, Eminance Advice` },
          { name: 'author', content: blog.author?.name || 'Eminance Advice' },
